import math
from causal_networkx.cgm import DAG
import pandas as pd 
...
def simulate_random_graphs_manm_cs(
    ...
    discrete_node_ratio : float, optional
        Defines the percentage of nodes that shall be of discrete type. Depending on its value the appropriate model (multivariate normal, mixed gaussian, discrete only) is chosen., by default 0.5
    discrete_signal_to_noise_ratio : float, optional
        Defines the probability that no noise is added within the mixed additive noise model., by default 0.9
    min_discrete_value_classes : int, optional
        Defines the minimum number of discrete classes a discrete variable shall have., by default 3
    ...
    with_conditional_gaussian : bool, optional
        If True defines that conditional gaussian model is assumed for a mixture of variables. Otherwise discrete variables can have continuous parents., by default True
    functions : str, optional
        A list of probabilities and mathmatical functions for relationships between two continuous nodes. Note, the input are tuples (probability, function), where the sum of all probabilities has to equal 1. Command line supported functions are: [linear, quadratic, cubic, tanh, sin, cos], by default 'linear'
    n_jobs : int, optional
    ...
    # if functions not in ALLOWED_FUNCTIONS:
    #     raise ValueError(f"Functions allowed are one of {ALLOWED_FUNCTIONS}.")
    # functions = [(1.0,lin_func)]
